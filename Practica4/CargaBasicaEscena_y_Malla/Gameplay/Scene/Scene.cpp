...
//Método que renderiza la escena recorriendo todas las mallas y llamando a los Render.
void cScene::Render()
{
	for (unsigned luiIndex = 0; luiIndex < mMeshList.size();++luiIndex)	
	{
		...
		// al recurso (malla en este caso) de cada manejador y se invoca el método RenderMesh() para cada malla.
        ((cMesh *)lpResource)->RenderMesh();
	}
}

//Método que libera la escena recorriendo todas las mallas y llamando a los Deinit.
void cScene::Deinit()
{   
   for ( cResourceHandleListIt lpIt = mMeshList.begin(); lpIt != mMeshList.end(); ++lpIt )
   {
      //Se recorre el vector de manejadores de malla accediendo (GetResource)
      // al recurso (malla en este caso) de cada manejador y se invoca el método Deinit() para cada malla.
